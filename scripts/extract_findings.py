...
- Computes sha256 over each `quote` for drift detection by downstream
  consumers (VS Code reviewer extension etc.).
...
def normalize(
    ...
    finding["section"] = str(finding["section"])
    finding["audit"] = audit_name
    finding["audit_md_line"] = audit_md_line
    finding["quote_sha256"] = hash_quote(str(finding["quote"]))
