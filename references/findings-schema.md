...
tags: [leakage:L3.2, reforms:6b, whalen:pitfall-2]
validator_pass:
  quote_match: true
  control_flow: true
...
- The cited `quote` can be re-checked against current file contents (drift
  detection) by hashing.
...
| Field | Type | Notes |
|---|---|---|
| `id` | string | Section-local slug, kebab-case, ≤ 40 chars. Stable across re-runs of the same audit. Globally namespaced as `<audit>/<id>` by the extractor. |
| `section` | string | Owning section number per Rule C (single-owner). One of `"0a"`, `"1"`, ..., `"16"`. |
...
| `status` | enum | `finding` \| `question`. Use `question` when evidence is incomplete (Rule A). |
...
| `claim` | string | What the code / artefact does (Rule G item 1). |
| `concern` | string | Why it is a concern (Rule G item 2). One sentence preferred. |
| `resolution` | string | A specific question for the authors or a specific check (Rule G item 4). |
...
| `cross_refs` | list[string] | Section / finding references (`["§4.2", "§9.1"]`). Empty list if none. |
...
| `tags` | list[string] | Taxonomy labels — see "Tag vocabulary" below. |
| `validator_pass` | object | Rule J self-check. Keys: `quote_match`, `control_flow`, `condition_satisfiable`, all booleans. All `true` for findings; if any are `false`, downgrade to `status: question`. |
...
file as a question with `status: question` (Rule A).

## Tag vocabulary

Tags are optional but make findings searchable across audits and let the
summary scoreboards aggregate by framework. Use lowercase, prefixed
`namespace:value`:

- `leakage:L1.1` ... `leakage:L3.3` — Kapoor-Narayanan taxonomy (Appendix C.2).
- `reforms:1` ... `reforms:8` — REFORMS modules (Appendix C.1).
- `whalen:pitfall-1` ... `whalen:pitfall-5` — Whalen pitfalls (Appendix C.3).
- `lones:stage-N` — Lones stages (Appendix C.4).
- `heil:bronze|silver|gold` — Heil reproducibility tier this finding moves the repo below.
- `forensics:post-hoc-selection|hidden-iteration|git-archaeology` — forensics sub-skill.
- `stats:grim|sprite|statcheck|carlisle|auc-ci|excessive-similarity` — statistical-integrity sub-skill that surfaced it.

## Schema-level invariants

1. **Single-owner**: a defect appears as a finding in exactly one section
   (the owner). Other sections cross-reference via `cross_refs`. The
   scoreboard must not double-count.
2. **Quote verbatim**: `quote` must be byte-equal to the cited range in the
   evidence location. Whitespace and indentation preserved. The extractor
   hashes `quote` and compares against the current file to detect drift.
...
cross_refs: ["§11"]
tags: [forensics:hidden-iteration, reforms:2]
```
